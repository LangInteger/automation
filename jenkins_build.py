...
def check_and_get_params():
    ...
    parser.add_argument("-e", "--env", choices=['dev', 'test'], required=True, help="environment, dev/test")
    parser.add_argument("-s", "--seq", choices=['1', '2', '3', '4', '5'], required=True, help="namespace sequence, 1/2/3/4")
    parser.add_argument("-n", "--name", required=True, help="service name, tmc-services")
...
def test_connection(build, args, cookies):
    url = build(args)
    logging.info("Build jenkins url: {0}".format(url))
    ...
    logging.info("Access jenkins with status: {0}".format(response.status_code))
